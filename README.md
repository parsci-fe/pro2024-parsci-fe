...
The coding software used to code the self-driving car the team has was AIcode. Since Nashenbot’s AISTEAM unit was used by the members to build the vehicle, that was the coding language that had to be used. The coding of the vehicle consists of modules that perform the appropriate actions and tasks the self-driving car must accomplish. The crucial components of the vehicle include a Camera or Visual sensor, one Full-color light module, one expansion screen module, two encoder motors (for moving forward and backward), one ordinary motor, one rudder module (for turning left and right), and one motor expansion module. Through the utilization of the Visual or camera sensor, managing through the obstacles became possible. The vehicle is also powered by AISTEAM’s lithium battery module and controller.
